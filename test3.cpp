...
const byte GARBAGE=255, LAST=9999;
...
void test_malloc_then_free() {
    ...
    for (int j=0; j<(int)BLOCK_SIZES[0]; ++j)
        *(p[0]+j) = DATA[0][j];
    ...
    assert_state(initial, expected);
    for (int j=0; j<(int)BLOCK_SIZES[1]; ++j)
        *(p[1]+j) = DATA[1][j];
    ...
    assert_state(initial, expected);
    for (int j=0; j<(int)BLOCK_SIZES[2]; ++j)
        *(p[2]+j) = DATA[2][j];
    ...
    assert_state(initial, expected);
    for (int j=0; j<(int)BLOCK_SIZES[3]; ++j)
        *(p[3]+j) = DATA[3][j];
...
void test_reuse_after_free() {
    ...
    for (int i=0; i<4; ++i) {
        ...
        for (int j = 0; j < (int)BLOCK_SIZES[i]; ++j) {
            *(p[i]+j) = DATA[i][j];
        }
    }
    assert_state(initial, expected);
    ...
    assert_state(initial, expected);
    for (int i=0; i<(int)BLOCK_SIZES[4]; ++i)
        *(p[4]+i) = DATA[4][i];
    ...
    *(p[0]) = 200;
    ...
    assert_state(initial, expected);
    for (int i=0; i<(int)BLOCK_SIZES[5]; ++i)
        *(p[5]+i) = DATA[5][i];
...
void test_realloc() {
    ...
    add_expected_block(expected, BLOCK_SIZES[2]);
    assert_state(initial, expected);
    for (int i=0; i<(int)BLOCK_SIZES[2]; ++i)
        p3[i] = DATA[2][i];
    assert(check_data(heap, DATA, 3, BLOCK_SIZES));
